...
class ZillowWrapper(object):
    ...
    def get_data(self, url, params):
        ...
        if response.findall('message/code')[0].text is not '0':
            ...
            response = ZillowError(int(response.findall('message/code')[0].text))
            return response
        else:
...
class GetDeepSearchResults(ZillowResults):
    ...
    attribute_mapping = {
        ...
        'zestimate_lastUpdated': 'result/zestimate/last-updated',
        'zestimate_valueChange': 'result/zestimate/valueChange',
        'zestimate_valuationRange_high':
        ...
        'rent_zestimate_amount': 'result/rentzestimate/amount',
        'rent_zestimate_lastUpdated': 'result/rentzestimate/last-updated',
        'rent_zestimate_valueChange': 'result/rentzestimate/valueChange',
        'rent_zestimate_valuationRange_high': 'result/rentzestimate/valuationRange/high',
        'rent_zestimate_valuationRange_low': 'result/rentzestimate/valuationRange/low',
        'home_value_index': 'result/localRealEstate/region/zindexValue',
    }
